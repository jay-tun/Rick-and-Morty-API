...
const create = async (req, res) =>{
    ...
    try{
        const q = `INSERT INTO characters
               ...
               RETURNING *`;
        const result = await pool.query(q, [name, species||null, status||null, 
            gender||null, origin||null, image||null, backstory||null, userId]);
...
const update = async (req, res) =>{
    ...
    try{
        //fetch existing
        const existingRes = await pool.query("SELECT * FROM characters WHERE id = $1", [id]);
        if (existingRes.rowCount === 0) return res.status(404).json({ error: "Not found" });
        const existing = existingRes.rows[0];
        if (existing.created_by !== userId) return res.status(403).json({ error: "Not allowed" });
...
const remove = async (req, res) =>{
    ...
    const userId = req.user.id;
    try{
        const ownerCheck = await pool.query("SELECT created_by FROM characters WHERE id = $1", [id]);
        if (ownerCheck.rowCount === 0) return res.status(404).json({ error: "Not found" });
        if (ownerCheck.rows[0].created_by !== userId) return res.status(403).json({ error: "Not allowed" });
